<?php
/**
 * Per-page header appearance overrides for Mindful Design Mega Menu.
 *
 ...
 * and the menu-slot renderer pass through the `md_mega_menu_is_transparent`
 * and `md_mega_menu_text_color` filters; this class subscribes
...
class MD_Mega_Menu_Page_Settings {

    const META_MODE  = '_md_mega_menu_header_mode';
    const META_COLOR = '_md_mega_menu_header_color';
    const NONCE      = 'md_mega_menu_page_settings';
    ...
    public static function init() {
        ...
        add_filter('md_mega_menu_is_transparent', array(__CLASS__, 'filter_transparent'), 10, 2);
        add_filter('md_mega_menu_text_color', array(__CLASS__, 'filter_color'), 10, 2);
    }

    public static function register_metabox() {
        foreach (array('page', 'post') as $post_type) {
            add_meta_box(
                'md-mega-menu-page-settings',
                __('Mega Menu Header', 'mindful-design-mega-menu'),
    ...
    public static function render_metabox($post) {
        ...
        ?>
        <p>
            <label for="md-mega-menu-header-mode">
                <strong><?php esc_html_e('Header style', 'mindful-design-mega-menu'); ?></strong>
            </label><br>
            <select name="md_mega_menu_header_mode" id="md-mega-menu-header-mode" style="width:100%;">
                <option value="<?php echo esc_attr(self::MODE_INHERIT); ?>" <?php selected($mode, self::MODE_INHERIT); ?>>
                    <?php esc_html_e('Inherit global setting', 'mindful-design-mega-menu'); ?>
                </option>
                <option value="<?php echo esc_attr(self::MODE_SOLID); ?>" <?php selected($mode, self::MODE_SOLID); ?>>
                    <?php esc_html_e('Solid (white background)', 'mindful-design-mega-menu'); ?>
                </option>
                <option value="<?php echo esc_attr(self::MODE_TRANSPARENT); ?>" <?php selected($mode, self::MODE_TRANSPARENT); ?>>
                    <?php esc_html_e('Transparent (over hero)', 'mindful-design-mega-menu'); ?>
                </option>
            </select>
        </p>
        <p>
            <label for="md-mega-menu-header-color">
                <strong><?php esc_html_e('Menu text colour', 'mindful-design-mega-menu'); ?></strong>
            </label><br>
            <input
                type="text"
                name="md_mega_menu_header_color"
                id="md-mega-menu-header-color"
                value="<?php echo esc_attr($color); ?>"
                placeholder="<?php esc_attr_e('e.g. #171717', 'mindful-design-mega-menu'); ?>"
                style="width:100%;"
            >
            <span class="description">
                <?php esc_html_e('Hex colour. Leave blank to auto-derive: white when the header is transparent, dark when it is solid.', 'mindful-design-mega-menu'); ?>
            </span>
        </p>
        <?php
    }
    ...
    public static function save_metabox($post_id, $post) {
        ...
        $allowed = array(self::MODE_INHERIT, self::MODE_SOLID, self::MODE_TRANSPARENT);
        $mode    = isset($_POST['md_mega_menu_header_mode'])
            ? sanitize_key(wp_unslash($_POST['md_mega_menu_header_mode']))
            : self::MODE_INHERIT;
        if (!in_array($mode, $allowed, true)) {
            $mode = self::MODE_INHERIT;
        }
        ...
        $color_raw = isset($_POST['md_mega_menu_header_color'])
            ? sanitize_text_field(wp_unslash($_POST['md_mega_menu_header_color']))
            : '';
